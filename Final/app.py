...
def login():
    if request.method == 'POST':
        ...
        return redirect(f'/dashboard/{usn}')

    return render_template('login.html')

@app.route('/dashboard/<string:usn>', methods=['GET', 'POST'])
def dashboard(usn):
    detail = db.find({'usn':usn})
    if detail==None:
        return render_template('dashboard.html', usn=usn, dev1='None', dev2='None')
    detail_ = detail["ip"]
    dev1=detail_[0]
    dev1[2]=dev1[2].strftime("%d/%m/%Y")
    try:
        dev2=detail_[1]
        dev2[2]=dev2[2].strftime("%d/%m/%Y")
    except:
        dev2='None'
    return render_template('dashboard.html', usn=usn, dev1=dev1, dev2=dev2)
